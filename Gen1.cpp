...
bool Gen1::pathCheck(int i,int start,int end, vector<int> &sciezka)
{
...
void Gen1::crossing(std::vector<int> &parent1, std::vector<int> &parent2) {
    int startR, endR;
...
double Gen1::start(double czas,int pop)
 {
   ...
   while(((clock()-start)/(CLOCKS_PER_SEC))<stop){
      ...
      for (int j = 0; j < (int) (mR * (float) nextPopulation.size()); j++) {
            do {
                index1 = rand() % macierz.size();
                index2 = rand() % macierz.size();
            } while (!(index1- index2));

            swap(nextPopulation.at(j)[index1], nextPopulation.at(j)[index2]);
        }
      
      int elita_size = popSize - nextPopulation.size(); 

         //sortowanie populacji by uzyskac najlepsze osobniki z obecnej
